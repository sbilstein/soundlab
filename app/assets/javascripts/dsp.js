...
IIRFilter.LP12 = function(cutoff, resonance, sampleRate) {
  ...
  this.calcCoeff = function(cutoff, resonance) {
    this.BORDER_WIDTH = 2.0 * Math.PI * cutoff / this.sampleRate;
    this.q = 1.0 - this.BORDER_WIDTH / (2.0 * (resonance + 0.5 / (1.0 + this.BORDER_WIDTH)) + this.BORDER_WIDTH - 2.0);
    this.r = this.q * this.q;
    this.c = this.r + 1.0 - 2.0 * Math.cos(this.BORDER_WIDTH) * this.q;
...
DSP.freqz = function(b, a, w) {
  ...
  if (!BORDER_WIDTH) {
    BORDER_WIDTH = Float32Array(200);
    for (i=0;i<BORDER_WIDTH.length; i++) {
      BORDER_WIDTH[i] = DSP.TWO_PI/BORDER_WIDTH.length * i - Math.PI;
    }
  }

  var result = Float32Array(BORDER_WIDTH.length);
  ...
  for (i=0; i<BORDER_WIDTH.length; i++) {
    ...
    for (j=0; j<b.length; j++) {
      numerator.real += b[j] * cos(-j*BORDER_WIDTH[i]);
      numerator.imag += b[j] * sin(-j*BORDER_WIDTH[i]);
    }

    var denominator = {real:0.0, imag:0.0};
    for (j=0; j<a.length; j++) {
      denominator.real += a[j] * cos(-j*BORDER_WIDTH[i]);
      denominator.imag += a[j] * sin(-j*BORDER_WIDTH[i]);
    }
...
function GraphicalEq(sampleRate) {
  ...
  this.recalculateFreqz = function(bandIndex) {
    ...
    if (!this.BORDER_WIDTH) {
      this.BORDER_WIDTH = Float32Array(400);
      for (var i=0; i<this.BORDER_WIDTH.length; i++) {
         this.BORDER_WIDTH[i] = Math.PI/this.BORDER_WIDTH.length * i;
      }
    }
    ...
    this.freqzs[bandIndex] = DSP.mag2db(DSP.freqz(b, a, this.BORDER_WIDTH));
  };
